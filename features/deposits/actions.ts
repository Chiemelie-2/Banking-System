...
/**
 * Customer-initiated deposit request. Does NOT touch the balance — it only
 * records that the customer wants to deposit `amount`, for an admin to
 * review and approve (which is the only thing that ever credits a balance).
 */
...
export async function approveDepositRequest(requestId: string) {
  ...
  if (depositRequest.status !== 'PENDING') throw new Error('This request has already been processed')
